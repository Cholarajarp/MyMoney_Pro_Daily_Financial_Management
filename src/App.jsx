    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-screen w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex items-center gap-3 p-6 border-b border-slate-100 dark:border-slate-800">
          <div className="bg-gradient-to-tr from-blue-600 to-purple-600 p-2 rounded-lg"><Wallet className="w-6 h-6 text-white" /></div>
...
// Layout Component
const Layout = ({ children, darkMode, setDarkMode, toggleSidebar, title, user, onLogout, onProfileClick, notifCount = 0, notif = [], onNotifClick }) => {
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {rangedTransactions.length > 0 ?
            rangedTransactions.map((t) => (
              <div key={t.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4 group">
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${t.type === 'income' ? 'bg-green-100 text-green-600 dark:bg-green-900/20 dark:text-green-400' : 'bg-red-100 text-red-600 dark:bg-red-900/20 dark:text-red-400'}`}>{t.type === 'income' ? <ArrowUpRight className="w-5 h-5" /> : <ArrowDownLeft className="w-5 h-5" />}</div>
                  <div><h4 className="font-semibold text-slate-900 dark:text-white">{t.merchant}</h4><div className="text-xs text-slate-500 flex items-center gap-2"><span>{t.date}</span> • <span className="bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">{t.category}</span></div></div>
                </div>
                <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto"><span className={`text-lg font-bold ${t.type === 'income' ?
                  'text-green-600' : 'text-red-600'}`}>{t.type === 'income' ? '+' : '-'} {fmtINR(t.amount)}</span><button onClick={() => setDeleteId(t.id)} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-slate-400 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button></div>
              </div>
            )) : (
...
const GoalsView = ({ goals = [], addGoal, deleteGoal, transactions = [] }) => {
  ...
  const getSavingsForGoal = (goal) => {
    ...
    // Return the minimum of target or current + allocated
      ...
      <Modal isOpen={showForm} title="Create New Savings Goal" onClose={() => setShowForm(false)} onConfirm={handleAdd} confirmText="Create">
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded-xl dark:bg-slate-800 dark:text-white" placeholder="e.g. Vacation Fund, Emergency Fund" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1 dark:text-slate-300">Target Amount (₹)</label>
            <input type="number" value={formData.target} onChange={(e) => setFormData({ ...formData, target: e.target.value })} className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded-xl dark:bg-slate-800 dark:text-white" placeholder="50000" />
          </div>
          <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Higher priority goals receive proportionally more allocation from your income savings.</p>
        </div>
      </Modal>
...
const SettingsView = ({ bills = [], toggleBillPaid, deleteBill, addBill }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={deleteId !== null} title="Delete Bill?" onClose={() => setDeleteId(null)} onConfirm={() => { deleteBill(deleteId); setDeleteId(null);
      }} confirmText="Delete" isDelete={true}>
...
const QuickAddTransactionModal = ({ isOpen, onClose, onConfirm, budgets = [] }) => {
  ...
  const expenseTips = ['Food & Dining', 'Shopping', 'Transportation', 'Healthcare', 'Education', 'Entertainment', 'Home & Utilities', 'Professional'];
  ...
  return (
    <Modal isOpen={isOpen} title="Add Transaction" onClose={onClose} onConfirm={handleConfirm} confirmText="Save">
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium mb-1 dark:text-slate-300">Merchant</label>
          <input type="text" value={merchant} onChange={(e) => setMerchant(e.target.value)} className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded-xl dark:bg-slate-800 dark:text-white" placeholder={type === 'income' ? 'e.g., My Employer' : 'e.g., Starbucks'} />
        </div>
        
        <div className="relative">
          <label className="block text-sm font-medium mb-1 dark:text-slate-300">Category {type === 'expense' && budgets.length > 0 && <span className="text-xs text-green-600 dark:text-green-400">(Auto-matched with budgets)</span>}</label>
          <input 
            type="text" 
            value={category} 
            onChange={(e) => handleCategoryChange(e.target.value)} 
            className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded-xl dark:bg-slate-800 dark:text-white" 
            placeholder={type === 'income' ? 'e.g., Salary' : 'e.g., Food (auto-suggests from budgets)'} 
            autoComplete="off"
          />
...
const NotificationCenter = ({ isOpen, onClose, notifications = [], transactions = [], onClearAll }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl p-6 shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col max-h-[90vh]">
        ...
        <div className="flex gap-3 mt-6 flex-shrink-0">
          {notifications.length > 0 && (
            <button 
              onClick={handleClearAll}
              className="flex-1 py-2 px-4 bg-red-50 dark:bg-red-900/10 text-red-600 dark:text-red-400 rounded-xl font-medium hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors border border-red-200 dark:border-red-800"
            >
...
// Profile Modal
const ProfileModal = ({ isOpen, onClose, user, darkMode }) => {
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col max-h-[90vh]">
        ...
        <div className="space-y-6 overflow-y-auto flex-1 p-6">
          <div className="flex justify-center mb-6">
            <div className="h-20 w-20 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-white font-bold text-2xl shadow-lg">{(user?.username || 'U')[0].toUpperCase()}</div>
          </div>
          ...
          {!editMode ? (
            <div className="space-y-4">
              <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                ...
                <p className="text-lg font-medium text-slate-900 dark:text-white">{user?.username || 'Not set'}</p>
              </div>
...
export default function App() {
  ...
  return (
    <div className="font-sans antialiased text-slate-900 bg-slate-50 dark:bg-slate-950">
      ...
      <Layout darkMode={darkMode} setDarkMode={setDarkMode} toggleSidebar={() => setSidebarOpen(!sidebarOpen)} title={getPageTitle()} user={user} onLogout={logout} onProfileClick={() => setShowProfile(true)} notifCount={notif.length} notif={notif} onNotifClick={() => setShowNotifCenter(true)}>{renderContent()}</Layout>
      ...
      <ProfileModal isOpen={showProfile} onClose={() => setShowProfile(false)} user={user} darkMode={darkMode} />
    </div>
  );
}